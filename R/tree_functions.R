...
nodeLogLike <- function(curr_part_res,
  ...
  if(length(node_basis_list)!=length(j_)){
    stop("Something wrong between the basis list and predictor VAR")
  }
  ...
  for(jj in 1:length(j_)){
      ...
      if(j_[jj] <= length(data$dummy_x$continuousVars)){
        cov_aux <- cov_aux + (data$tau_beta[tree_number,j_[jj]]^(-1))*node_basis_list[[jj]][,,drop = FALSE]%*%tcrossprod(data$P_inv,(node_basis_list[[jj]][,,drop = FALSE]))
      } else {
        cov_aux <- cov_aux + (data$tau_beta[tree_number,j_[jj]]^(-1))*node_basis_list[[jj]][,,drop = FALSE]%*%tcrossprod(data$P_inv_interaction,(node_basis_list[[jj]][,,drop = FALSE]))
      }
  }
...
grow <- function(tree,
  ...
  B_list <- new_basis_list(pred_vars_ = node_index_var,master_var_ = g_node$master_var,
                           train_observations_index = g_node$train_index,
                           data = data, pen_basis = TRUE)
  B_list_left <- new_basis_list(pred_vars_ = node_index_var,master_var_ = g_node$master_var,
                                train_observations_index = left_index,
                                data = data, pen_basis = TRUE)
  B_list_right <- new_basis_list(pred_vars_ = node_index_var,master_var_ = g_node$master_var,
                                 train_observations_index = right_index,
                                 data = data, pen_basis = TRUE)
...
add_variable <- function(tree,
  ...
  # Getting the basis functions for the new basis
  B_current_node <- new_basis_list(pred_vars_ = g_node$pred_vars,
                                   master_var_ = g_node$master_var,
                                   train_observations_index = g_node$train_index,
                                   data = data,pen_basis = TRUE)

  B_new <- new_basis_list(pred_vars_ = new_node_index_var,
                                   master_var_ = g_node$master_var,
                                   train_observations_index = g_node$train_index,
                                   data = data,pen_basis = TRUE)


  g_loglike <- nodeLogLike(curr_part_res = curr_part_res,node_basis_list = B_current_node,
                               j_ = g_node$pred_vars,
                               index_node = g_node$train_index,
                               data = data,tree_number = tree_number)


  new_g_loglike <-  nodeLogLike(curr_part_res = curr_part_res,node_basis_list = B_new,
...
prune <- function(tree,
  ...
  # List of new basis_functions
  B_list <- new_basis_list(pred_vars_ = node_index_var,master_var_ = p_node$master_var,
                           train_observations_index = p_node$train_index,
                           data = data, pen_basis = TRUE)
  B_list_left <- new_basis_list(pred_vars_ = node_index_var,master_var_ = p_node$master_var,
                                train_observations_index = children_left_index,
                                data = data, pen_basis = TRUE)
  B_list_right <- new_basis_list(pred_vars_ = node_index_var,master_var_ = p_node$master_var,
                                 train_observations_index = children_right_index,
                                 data = data, pen_basis = TRUE)


  p_loglike <- nodeLogLike(curr_part_res = curr_part_res,node_basis_list = B_list,
                           index_node = p_node$train_index,
                           j_ = node_index_var,
                           data = data,tree_number = tree_number)


  p_left_loglike <-  nodeLogLike(curr_part_res = curr_part_res,node_basis_list = B_list_left,
                                 index_node =  children_left_index,
                                 j_ = node_index_var,
                                 data = data,tree_number = tree_number)

  p_right_loglike <-  nodeLogLike(curr_part_res = curr_part_res,node_basis_list = B_list_right,
...
remove_variable <- function(tree,
  ...
  # Getting the new basis
  B_current_node <- new_basis_list(pred_vars_ = p_node$pred_vars,master_var_ = p_node$master_var,
                                   train_observations_index = p_node$train_index,data = data,
                                   pen_basis = TRUE)

  B_new <- new_basis_list(pred_vars_ = new_node_index_var,master_var_ = p_node$master_var,
                          train_observations_index = p_node$train_index,data = data,
                          pen_basis = TRUE)

  # Getting the one for the current node
  p_loglike <- nodeLogLike(curr_part_res = curr_part_res,
                           node_basis_list = B_current_node,
                           j_ = p_node$pred_vars,
                           index_node = p_node$train_index,
                           data = data,tree_number = tree_number)
...
change <- function(tree,
  ...
  # Generating the left and the right nodes basis
  B_list_left <- new_basis_list(pred_vars_ = node_index_var,master_var_ = c_node$master_var,
                                train_observations_index = old_left_index,data = data,
                                pen_basis = TRUE)

  B_list_right <- new_basis_list(pred_vars_ = node_index_var,master_var_ = c_node$master_var,
                                 train_observations_index = old_right_index,data = data,
                                 pen_basis = TRUE)



  # Calculating loglikelihood for the new changed nodes and the old ones
  c_loglike_left <- nodeLogLike(curr_part_res = curr_part_res,node_basis_list = B_list_left,
                                    index_node = old_left_index,
                                    j_ = node_index_var,
                                    data = data,tree_number = tree_number)


  c_loglike_right <-  nodeLogLike(curr_part_res = curr_part_res,node_basis_list = B_list_right,
  ...
  B_list_left_new <- new_basis_list(pred_vars_ = node_index_var,master_var_ = c_node$master_var,
                                    train_observations_index = left_index,data = data,
                                    pen_basis = TRUE)

  B_list_right_new <- new_basis_list(pred_vars_ = node_index_var,master_var_ = c_node$master_var,
                                     train_observations_index = right_index,data = data,
                                     pen_basis = TRUE)

  new_c_loglike_left <-  nodeLogLike(curr_part_res = curr_part_res,
                                     node_basis_list = B_list_left_new,
                                     index_node = left_index,
                                     j = node_index_var,
                                     data = data,tree_number = tree_number)

  new_c_loglike_right <-  nodeLogLike(curr_part_res = curr_part_res,
                                      node_basis_list = B_list_right_new,
                                      index_node = right_index,
                                      j =  node_index_var,
                                      data = data,tree_number = tree_number)
...
change_stump <- function(tree,
  ...
  # Generating new basis function
  B_list <- new_basis_list(pred_vars_ = c_node$pred_vars,
...
updateBetas <- function(tree,
  ...
  for(i in 1:length(t_nodes_names)){
    ...
    # Generating all the basis for the node_index
    B_list <- new_basis_list(pred_vars_ = node_index_var,
                             master_var_ = cu_t$master_var,
                             train_observations_index = cu_t$train_index,
                             data = data,pen_basis = TRUE)

    B_list_test <- new_basis_list_test(pred_vars_ = node_index_var,
                                       master_var_ = cu_t$master_var,
                                       train_observations_index = cu_t$train_index,
                                       test_observations_index = cu_t$test_index,
                                       data = data,pen_basis = TRUE)
    ...
    for(jj in 1:length(node_index_var)){
      ...
      if(length(other_leaf_basis_subindex)!=0){
        for(iter_minus_jj in 1:length(other_leaf_basis_subindex)){
          ...
          pred_minus_jj <- pred_minus_jj + tcrossprod(B_list[[as.character(other_leaf_basis_subindex[iter_minus_jj])]],current_betas)
        }
      ...
      b_ <- crossprod(B_list[[jj]],res_leaf)


      if(node_index_var[jj]<=length(data$dummy_x$continuousVars)){
        U_ <- data$P*data$tau_beta[tree_number,node_index_var[jj]]
      } else {
        U_ <- data$P_interaction*data$tau_beta[tree_number,node_index_var[jj]]
      }

      Q_ <- (crossprod(B_list[[jj]]) + U_)
      ...
      new_partial_pred <- tcrossprod(B_list[[jj]],new_betas) #+ res_leaf_mean
      ...
      y_hat_test[cu_t$test_index,node_index_var[jj]] <- tcrossprod(B_list_test[[jj]],new_betas) #+ res_leaf_mean
    }
...
# Update \tau_betas
# =================
update_tau_betas_j <- function(tree,
                               data){
    ...
    for(curr_node in terminal_nodes_names) {
      ...
      for(k in pred_vars){
        ...
        if(k <= length(data$dummy_x$continuousVars)){
          j_vec[k] <- j_vec[k] + data$tau_beta[tree,k]*current_betas%*%crossprod(data$P,current_betas)
        } else {
          j_vec[k] <- j_vec[k] + data$tau_beta[tree,k]*current_betas%*%crossprod(data$P_interaction,current_betas)
        }
      }
